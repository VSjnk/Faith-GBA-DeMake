...
#define  SH      80 
#define  posX     0                                        //game screen width
#define  posY     0                                        //game screen height
...
void updatePlayer()
{
	...
	if(dir==0 && KEY_R){if(P.frame==0){P.map=pVert1_Map;}if(P.frame==1){P.map=pVert2_Map;} P.rx=6; P.ry=10;} else{if (dir==0){P.map=pVertIdle_Map; P.rx=5; P.ry=10;}}
	if(dir==0 && !KEY_R && KEY_A){P.map=pVertAtk_Map; P.rx=6; P.ry=10;}
	
	if(dir==1 && KEY_L){if(P.frame==0){P.map=pVert1_Map;}if(P.frame==1){P.map=pVert2_Map;} P.rx=6; P.ry=10;} else{if (dir==1){P.map=pVertIdle_Map; P.rx=5; P.ry=10;}}
	if(dir==1 && !KEY_L && KEY_A){P.map=pVertAtk_Map; P.rx=6; P.ry=10;}
	
	if(dir==2 && KEY_U){P.map=pUp_Map; P.rx=6; P.ry=10;} else{if (dir==2){P.map=pUpIdle_Map; P.rx=5; P.ry=10;}}
	if(dir==2 && !KEY_U && KEY_A){P.map=pUpAtk_Map; P.rx=5; P.ry=10;}
	
	if(dir==3 && KEY_D){P.map=pDown_Map; P.rx=6; P.ry=10;} else{if (dir==3){P.map=pDownIdl_Map; P.rx=5; P.ry=10;}}
...
void tutDemon(int x, int y)
{
	...
	if(dir==0 && KEY_A && x>P.x){drawImage(7,10, x,y, DemonTut_Map, 1); died=1; }
	if(dir==1 && KEY_A && x<P.x){drawImage(7,10, x,y, DemonTut_Map, 1); died=1; }
	if(dir==2 && KEY_A && y<P.y){drawImage(7,10, x,y, DemonTut_Map, 1); died=1; }
	if(dir==3 && KEY_A && y>P.y){drawImage(7,10, x,y, DemonTut_Map, 1); died=1; }
 ...
 *(u16*)0x4000000 = 0x405;                                           //mode 5 background 2
 ...
 { 
   ...
   buttons();                                                        //button input
   updatePlayer();
   if(D.demon==0){apple(60, 40);}
   if(D.demon==1){apple(43, 60);}
   if(D.demon==2){apple(95, 50);}
   if(D.demon>=3){drawImage(120, 17, 0, 14, CastOut_Map, 0);}
   if(D.demon==3){tutDemon(70, 40);}
   if(D.demon==4){tutDemon(55, 50);}
   if(D.demon==5){tutDemon(40, 47);}
